...
const BlockImageText = (props: ToolRenderProps) => {
  const [list, setList] = useState<Array<any>>(
    props.blockdata.children ? props.blockdata.children : [],
  );
  ...
  return (
    <div
      ...
      className={getCommonBlockCss('imagetext', styleIdentifier) + ' ' + imagetextStyle}
    >
      {props.active && (
        <BlockProperty blocktype="imagetext" inBlock={props.inBlock}>
      ...
      <div
        className="dme-imagetext-container"
        style={!isMobile ? { display: 'flex', flexWrap: flexWrap } : {}}
      >
        {list.map((item: any, index: any) => {
          return (
            <React.Fragment key={item.id ? item.id : index}>
              {item.type == 'list' && (
                <div style={{ width: 'calc(100% - ' + imageWidth + ')' }}>
                  <BlockList
                    view={props.view}
                    allowedType={['text', 'image', 'heading']}
                    onChange={(data) => onChange({ ...list[index], children: data }, index)}
              ...
              data: [
                {
                  ...
                  children: [
                    { text: '' },
                    {
                      ...
                      styleConfig: {
                        style: 'button',
                        setting: { size: 'small', color: 'primary', variant: 'outlined' },
                      },
                    },
                    { text: '' },
                  ],
