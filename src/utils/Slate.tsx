...
export const SlateFun: any = {
  ...
  toggleFormat: (editor: any, format: string, value?: any) => {
    ...
    let property = SlateFun.TEXT_FORMAT_TYPES.includes(format)
      ? "type"
      : "style";
    let newProperty: any;
  ...
  FormatButton: ({ formats, changeDialogLink }: any) => {
    ...
    if (formats === "bold") {
      ele = (
        <FormatBold
          sx={{
            color: SlateFun.isFormatActive(editor, formats) ? "white" : "#aaa",
          }}
  ...
  resetTypes: (type: any, config: any) => {
    let arr: any;
    arr = config
      ? SlateFun[type].filter((item: any) => config.includes(item))
      : SlateFun[type];
    return arr;
  },
  //star new 20221021--s
  ...
  unwrapLink: (editor: any) => {
    Transforms.unwrapNodes(editor, {
      match: (n: any) =>
        n.type === "link" && !Editor.isEditor(n) && SlateElement.isElement(n),
    });
  },
  ...
  LinkComponent: ({ attributes, children, element }: any) => {
    ...
    if (element.source.sourceType === "select") {
      link =
        "{link:" +
        element.source.sourceData.location.content_type +
        "," +
        element.url +
        "}";
    }
  ...
  ImageComponent: ({
    ...
    if (element.setting) {
      ...
      let borderColor =
        element.setting?.borderColor || SlateFun.IMAGE_BORDER_COLOR;
      Commonstyle = {
        ...Commonstyle,
      };
      ...
      <>
        ...
        {!view && (
          <ReactResizable
            ...
            onChange={(size: any) => {
              let newProperty: any;
              let setting: any = element?.setting
                ? { ...element.setting, ...size }
                : { ...size };
              newProperty = {
                setting: { ...setting },
              };
              ...
              >
                ...
                <FormatAlignLeft
                  className={css`
                    ...
                    color: ${element?.float && element.float == "left"
                      ? "#12913e"
                      : "#333"};
                    cursor: pointer;
                  `}
                ...
                <FormatAlignRight
                  className={css`
                    ...
                    background-color: white !important;
                    color: ${element?.float && element.float == "right"
                      ? "#12913e"
                      : "#333"};
                    cursor: pointer;
                  `}
  ...
  isImageActive: (editor: any) => {
    const [image] = Editor.nodes(editor, {
      match: (n: any) =>
        n.type === "image" && !Editor.isEditor(n) && SlateElement.isElement(n),
    });
    return !!image;
  },
  ...
  ButtonComponent: ({ attributes, children, element }: any) => {
    return (
      <Button
        color="success"
        size={
          element.hasOwnProperty("setting") ? element.setting.size : "small"
        }
        variant={
          element.hasOwnProperty("setting")
            ? element.setting.variant
            : "outlined"
        }
        {...attributes}
  ...
  unwrapButton: (editor: any) => {
    Transforms.unwrapNodes(editor, {
      match: (n: any) =>
        n.type === "button" && !Editor.isEditor(n) && SlateElement.isElement(n),
    });
  },
